...
@shared_task
def add(x, y):
    experimental.apply_async([range(10)])
    return x + y
...
def convert_media_elastic(media_id, pipeline_name, update_media=True):
    ...
    logger.warning("Launched Polling for: %s", media_id)
...
def poll_elastic_transcoder(media_id, job_id, update_media=True):
    ...
    logger.warning("Polling media status, %s, is how it is.",
                   output['Status'])
    ...
    if output['Status'] in ('Submitted', 'Progressing'):
        ...
        raise poll_elastic_transcoder.retry(countdown=10)
    elif job['Status'] == 'Complete':
        if update_media:
            media.status = 'C'
            media.set_internal_file_s3_key(job['OutputKeyPrefix'] + output['Key'])
    else:
